...
async function main() {
	...
	if (hre.network.name == 'sepolia') {
		...
		// add Payment Tokens
		tx = await cryptocommoditiesFactory.setPaymentToken("WBTC", wbtc.address, "0x1b44F3514812d835EB1BDB0acB33d3fA3351Ee43", Math.floor(DEF_PRICE_BTC_IN_USD * 10**6), 18); tx.wait();
		tx = await cryptocommoditiesFactory.setPaymentToken("COIN", cryptocommoditiesFactory.address, "0x694AA1769357215DE4FAC081bf1f309aDC325306", Math.floor(DEF_PRICE_ETH_IN_USD * 10**6), 18); tx.wait();
		tx = await cryptocommoditiesFactory.setPaymentToken("MATIC", matic.address, ZERO_ADDRESS, Math.floor(DEF_PRICE_MATIC_IN_USD * 10**6), 18); tx.wait();
		tx = await cryptocommoditiesFactory.setPaymentToken("BNB", bnb.address, ZERO_ADDRESS, Math.floor(DEF_PRICE_BNB_IN_USD * 10**6), 18); tx.wait();
		tx = await cryptocommoditiesFactory.setPaymentToken("USDT", usdt.address, ZERO_ADDRESS, Math.floor(DEF_PRICE_USDT_IN_USD * 10**6), 18); tx.wait();
	...
	if (hre.network.name == 'polygon_mumbai') {
		...
		tx = await cryptocommoditiesFactory.setPaymentToken("WBTC", '0x0d787a4a1548f673ed375445535a6c7A1EE56180', '0x007A22900a3B98143368Bd5906f8E17e9867581b', Math.floor(DEF_PRICE_BTC_IN_USD * 10**6), 8); tx.wait();
...
1
		...
		tx = await cryptocommoditiesFactory.setPaymentToken("WETH", '0xA6FA4fB5f76172d178d61B04b0ecd319C5d1C0aa', "0x0715A7794a1dc8e42615F059dD6e406A6594651A", Math.floor(DEF_PRICE_ETH_IN_USD * 10**6), 18); tx.wait();
		console.log("WETH installed");

		// deploy MATIC
		// https://faucet.polygon.technology/
		tx = await cryptocommoditiesFactory.setPaymentToken("COIN", cryptocommoditiesFactory.address, "0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada", Math.floor(DEF_PRICE_MATIC_IN_USD * 10**6), 18); tx.wait();
		console.log("COIN installed");
		...
		const bnb = await BNB.deploy("BNB", "BNB");
		await bnb.deployed();
		tx = await cryptocommoditiesFactory.setPaymentToken("BNB", bnb.address, chainLinkAggregator.address, Math.floor(DEF_PRICE_BNB_IN_USD * 10**6), 18); tx.wait();
		...
		tx = await cryptocommoditiesFactory.setPaymentToken("USDT", '0xa02f6adc7926efebbd59fd43a84f4e0c0c91e832', '0x92C09849638959196E976289418e5973CC96d645', Math.floor(DEF_PRICE_USDT_IN_USD * 10**6), 6); tx.wait();
