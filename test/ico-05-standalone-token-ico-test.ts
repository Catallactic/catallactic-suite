...
describe("ico-05-standalone-token-ico-test", function () {
	...
	it("Should be able to whitelist and unwhitelist", async() => {
		...
		await expect(helpers.testTransferToken(addr1, 'FOO', 31, ico, foo)).to.be.revertedWith('ERRD_MUST_WHI');
	...
	it("Should be able to blacklist and unblacklist", async() => {
		...
		await ico.blacklistUser(addr1.address);
		await expect(helpers.testTransferToken(addr1, 'FOO', 10, ico, foo)).to.be.revertedWith('ERRD_MUSN_BLK');
	...
	it("Should respect transfer limits", async() => {
		...
		await expect(helpers.testTransferToken(addr1, 'FOO', 9, ico, foo)).to.be.revertedWith('ERRD_TRAS_LOW');
		await expect(helpers.testTransferToken(addr1, 'FOO', 10, ico, foo)).not.to.be.reverted;
		await ico.setMinuUSDTransfer(9.99 * 10**6);

		await ico.setMaxuUSDTransfer(20.0001 * 10**6);
		await expect(helpers.testTransferToken(addr1, 'FOO', 21, ico, foo)).to.be.revertedWith('ERRD_TRAS_HIG');
		...
		await expect(helpers.testTransferToken(addr1, 'FOO', 10, ico, foo)).not.to.be.reverted;
		await expect(helpers.testTransferToken(addr1, 'FOO', 10, ico, foo)).not.to.be.reverted;
		await expect(helpers.testTransferToken(addr1, 'FOO', 40, ico, foo)).to.be.revertedWith('ERRD_INVT_HIG');
